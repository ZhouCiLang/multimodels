...
def evaluate_dataset(model, device, params, database_sets, query_sets, silent=True):
    ...
    for m in tqdm.tqdm(range(len(query_sets)), disable=silent):
        for n in range(len(query_sets)):
            if m == n:
                continue
            database_output = database_embeddings[m]
            queries_output = query_embeddings[n]
            ...
            for i in range(len(queries_output)):
                ...
                print('indices[0]',indices.shape)
                print('query\n:', query_details)
                print('database1:\n', database_sets[m][indices[0, 0]])
                print('database2:\n', database_sets[m][indices[0, 1]])
                print('database3:\n', database_sets[m][indices[0, 2]])
        
#             pair_recall, pair_similarity, pair_opr,pair_opr1 = get_recall(i, j, database_embeddings, query_embeddings, query_sets,
#                                                                 database_sets)
#             recall += np.array(pair_recall)
#             count += 1
#             one_percent_recall.append(pair_opr)
#             one_recall.append(pair_opr1)
...
#     ave_one_recall = np.mean(one_recall)
#     stats = {'ave_one_percent_recall': ave_one_percent_recall, 
#              'ave_one_recall':ave_one_recall,
...
def get_recall(m, n, database_vectors, query_vectors, query_sets, database_sets):
    ...
    for i in range(len(queries_output)):
        ...
        if len(list(set(indices[0][0:threshold1]).intersection(set(true_neighbors))))>0:
            one_retrieved+=1
            
    one_percent_recall = (one_percent_retrieved/float(num_evaluated))*100
    one_recall = (one_retrieved/float(num_evaluated))*100 
    
    recall = (np.cumsum(recall)/float(num_evaluated))*100
    return recall, top1_similarity_score, one_percent_recall, one_recall


def print_eval_stats(stats):
    for database_name in stats:
        print('Dataset: {}'.format(database_name))
        t = 'Avg. top 1% recall: {:.2f}  Avg. top 1 recall: {:.2f}   Avg. similarity: {:.4f}   Avg. recall @N:'
        print(t.format(stats[database_name]['ave_one_percent_recall'],stats[database_name]['ave_one_recall'], stats[database_name]['average_similarity']))
