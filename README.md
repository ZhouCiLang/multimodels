# multimodal (forked) — 项目说明

这是仓库的工作副本，包含点云（PC）与 RGB 相关训练代码与脚本。

- 数据目录示例：`/root/autodl-tmp/benchmark_datasets`
- 训练脚本：`train.py`，配置位于 `config/`。
...
# Generate training tuples for the Baseline Dataset
python generate_rgb_for_lidar.py --config ../config/config_baseline_multimodal.txt --oxford_root /userhome/datasets/benchmark_datasets/oxford
```
