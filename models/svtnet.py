...
class csvt_module(nn.Module):
    ...
    def forward(self, x):
        ...
        x_r = ME.SparseTensor(features=x_r,
...
class SVTNet(ResNetBase):
    ...
    def network_initialization(self, in_channels, out_channels, D):
        ...
        for plane, layer in zip(self.planes, self.layers):
            ...
            ## modified here
            bloc = self._make_layer(self.block, plane, layer)
...
class SVTQI(ResNetBase):
    ...
    def network_initialization(self, in_channels, out_channels, D):
        ...
        self.conv1x1 = nn.ModuleList()  # 1x1 convolutions in lateral connections
#         self.conv1x1_r = nn.ModuleList()  # 1x1 convolutions in lateral connections
#         self.conv1x1_i = nn.ModuleList()  # 1x1 convolutions in lateral connections
        
        self.amp= ME.MinkowskiConvolution(in_channels, in_channels, kernel_size=1, stride=1, dimension=D)
        
        self.pha= ME.MinkowskiConvolution(in_channels, in_channels, kernel_size=1, stride=1, dimension=D)
        
        self.cos= ME.MinkowskiConvolution(in_channels, in_channels, kernel_size=1, stride=1, dimension=D)
        self.sin= ME.MinkowskiConvolution(in_channels, in_channels, kernel_size=1, stride=1, dimension=D)
        
        # The first convolution is special case, with kernel size = 5
        self.inplanes = self.planes[0]
#         self.conv0 = ME.MinkowskiConvolution(in_channels, self.inplanes, kernel_size=self.conv0_kernel_size,
#                                              dimension=D)
        self.conv0_r = ME.MinkowskiConvolution(in_channels, self.inplanes, kernel_size=self.conv0_kernel_size,
                                             dimension=D)
        self.conv0_i = ME.MinkowskiConvolution(in_channels, self.inplanes, kernel_size=self.conv0_kernel_size,
                                             dimension=D)
        self.bn0 = ME.MinkowskiBatchNorm(self.inplanes)
        ...
        self.conv1x1.append(ME.MinkowskiConvolution(self.inplanes,self.lateral_dim, kernel_size=1,
                                                    stride=1, dimension=D))
#         self.conv1x1_r.append(ME.MinkowskiConvolution(self.inplanes,self.lateral_dim, kernel_size=1,
#                                                     stride=1, dimension=D))
#         self.conv1x1_i.append(ME.MinkowskiConvolution(self.inplanes,self.lateral_dim, kernel_size=1,
#                                                     stride=1, dimension=D))
        ...
        self.asvt = asvt_module(self.lateral_dim ,reduction)
        self.csvt = csvt_module(self.lateral_dim, 8)
        self.conv1x1p1= ME.MinkowskiConvolution(self.lateral_dim, self.lateral_dim, kernel_size=1, stride=1, dimension=D)
    ...
    def forward(self, x):
        ...
        amp = self.amp(x)
        amplitude = F.normalize(amp, dim=-1)
        
        pha = self.pha(x)
        phase = self.sigmoid(pha)
        
        x_r = self.cos(phase)*amplitude
        x_i = self.sin(phase)*amplitude
