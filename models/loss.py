...
from models.truncated_smoothap import TruncatedSmoothAP, MultiTruncatedSmoothAP

def make_losses(params):
    ...
    elif params.loss == 'MultiBatchHardContrastiveLoss':
        # BatchHard mining with triplet margin loss
        # Expects input: embeddings, positives_mask, negatives_mask
        loss_fn = MultiBatchHardContrastiveLossWithMasks(params.pos_margin, params.neg_margin, params.normalize_embeddings, params.weights)    
    ...
    elif params.loss == 'MultiTruncatedsmoothap':
        loss_fn = MultiTruncatedSmoothAP(params.tau1, params.similarity, params.positives_per_query, params.weights)
    else:
...
class MultiBatchHardContrastiveLossWithMasks:
    def __init__(self, pos_margin, neg_margin, normalize_embeddings, weights):
        self.pos_margin = pos_margin
        self.neg_margin = neg_margin
        assert len(weights) == 3
        self.weights = weights
        self.final_loss = BatchHardContrastiveLossWithMasks(pos_margin, neg_margin, normalize_embeddings)
        self.cloud_loss = BatchHardContrastiveLossWithMasks(pos_margin, neg_margin, normalize_embeddings)
        self.image_loss = BatchHardContrastiveLossWithMasks(pos_margin, neg_margin, normalize_embeddings)
        print('MultiBatchHardContrastiveLoss')
        print('Weights (final/cloud/image): {}'.format(weights))
