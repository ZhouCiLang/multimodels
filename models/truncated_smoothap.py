...
class TruncatedSmoothAP:
    ...
    def __call__(self, x, positives_mask, negatives_mask):
        ...
        s_qz = compute_aff(x, similarity=self.similarity)
        ...
        stats['ap'] = ap.item()
        stats['avg_embedding_norm'] = x.norm(dim=1).mean().item()
        return loss, stats, None


class MultiTruncatedSmoothAP:
    def __init__(self, tau1, similarity, positives_per_query, weights):
        assert len(weights) == 3
        self.weights = weights
        self.final_loss = TruncatedSmoothAP(tau1, similarity, positives_per_query)
        self.cloud_loss = TruncatedSmoothAP(tau1, similarity, positives_per_query)
        self.image_loss = TruncatedSmoothAP(tau1, similarity, positives_per_query)
        print('MultiTruncatedSmoothAP')
